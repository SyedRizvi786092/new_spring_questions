...
//		return events.stream().map(EventMapper::toResponse).toList(); // empty list if none
		...
		List<Event> result = repo.findAll()
				.stream()
	...
	public PurchaseResponseDTO purchaseApi( PurchaseDTO pdto){
		Optional<Event> data = repo.findById(pdto.getId());
		if(!data.isPresent()) {
			//throw Exception
		}
		
		Event event = data.get();
		if(pdto.getQuantity() > event.getAvailableTickets()) {
			//throw Exception
		}
		
		double totalPrice = pdto.getQuantity() * event.getPrice().doubleValue();
		event.setAvailableTickets(event.getAvailableTickets()-pdto.getQuantity());
		repo.save(event);
		
		return new PurchaseResponseDTO(pdto.getId(),pdto.getName(),new BigDecimal(totalPrice));
